...
\emph{Identification and units.} The informational current is the entropy current, $j\equiv j_S$
(nats/time), and the informational density is the entropy density, $i\equiv s$ (nats/vol).
Where a boundary temperature field $T$ is assigned, the heat flux is $q:=T\,j_S$.
...
\]
Differentiating \eqref{appB:eq:stokes-int} in $t$ yields the \emph{cut identity}
\begin{equation}
\dot I_{\mathrm{enc}}(t)\;=\;\Phi_{\mathrm{in}}(t)-\Phi_{\mathrm{out}}(t)\;+\;\Pi(t),
\label{appB:eq:cut}
\end{equation}
identical to \eqref{eq:cut-identity} in the main text.
...
the kinematic term $\mathbf j\!\cdot n - i\,v_n$, with caps evaluated on the initial and final slices. This is the
form used when $A_{n}(r,t)$ varies in time.
...
\label{appB:static}
For a time–frozen snapshot, on an open set $W\subset U$ with $\sigma|_W\equiv 0$ (source‑free), $d j^{(n)}=0$ on $W$
implies that the flux through any two homologous screens in $W$ is equal. If $\mathcal T$ is a narrow tube whose side
surface is tangent to $j$, then the directed tube flux $\Phi=\int_{\Sigma}j\!\cdot n\,dA$ is constant along the tube
and cross–sectional averages take the form
\[
...
constitutive map $\chi$ on the segment considered. \emph{When a causal, time–invariant model for $\chi$ is adopted—or
when retarded response is observed on a window—the measured/adopted dispersion is compared with the
Kramers--Kronig Hilbert–transform relations recorded in \S\ref{appB:KK}, eq.~\eqref{appB:eq:KK}, as a \emph{comparison witness} on that window.}
\emph{General wave transport.} The same energy–flux balance applies to any pair $(U,V)$ with $V=\chi:U$,
...
\label{appB:KK}
For linear, time–invariant media the (temporal) Fourier transform $\chi(\omega)$ satisfies the standard
...
\label{appB:eq:KK}
\end{align}
These relations are recorded here as a comparison template: when retardedness is observed on a window, empirical
dispersion may be checked against \eqref{appB:eq:KK} as a local witness.
...
(i) the local continuity equation \eqref{appB:eq:local}, and (ii) the cut identity \eqref{appB:eq:cut} from the
