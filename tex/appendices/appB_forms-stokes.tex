...
in sectors where dissipation is present.
...
form used when the screen area $A_{n-1}(r,t)$ varies in time.
...
and cross–sectional averages obey
\[
...
constitutive map $\chi$ on the segment considered. \emph{When a causal model for $\chi$ is adopted, or when
retarded response is observed on a window}, the measured/adopted transfer may be \emph{compared} against the
Kramers--Kronig Hilbert–transform relations recorded in \S\ref{appB:KK}, eq.~\eqref{appB:eq:KK}. This is a
diagnostic/witness for that regime, not a global postulate.
