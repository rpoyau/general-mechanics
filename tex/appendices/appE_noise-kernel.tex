...
\end{equation}
On a \emph{boundary patch} \(A\subset\partial U\) with outward unit normal \(n\), one may equivalently
...
Linearize the channel’s geometric/constitutive response around a background that \emph{follows} the
mean continuity relation (from the Stokes identity; App.~\ref{app:forms-stokes},
...
\end{equation}
For \emph{boundary‑patch} estimators built from integrals over \(A\subset\partial U\) (e.g., of \(\mathbf j\!\cdot n\)),
\eqref{app:noise-kernel:eq:cov-prop} reduces to patch–filtered versions with the corresponding projected kernel.

% ------------------------- E.4: Boundary variance floor ----------------

\subsection{Boundary/patch estimator: variance floor (windowed)}
...
\[
j_A:=\frac{\mathbb E[N_A]}{|A|\,\Delta t},
\]
the windowed \emph{Fano factor}
\[
F_A:=\frac{\mathrm{Var}(N_A)}{\mathbb E[N_A]}\quad\text{(same patch and window)},
\]
and an \emph{effective window} \(\Delta t_{\!\mathrm{eff}}\le \Delta t\) obtained from the
\emph{count autocorrelation} on the same record (App.~\ref{app:noise-kernel}).
Then the informational–field estimator admits the windowed benchmark
\begin{equation}
...
the same scaling transfers to measured sectors. (This matches the estimator floor in \S\ref{operationalization:noise}.)
...
\)
In a shot–noise regime, \(D_Y\) inherits the same \(\propto j_A/|A|\) scaling as
...
\(
\mathrm{Var}(\widehat{E}_I)\gtrsim \frac{j_A}{\kappa^2|A|}\,\frac{\tau}{\Delta t}.
\)
...
This matches \eqref{app:noise-kernel:eq:EI-bound} upon identifying \(\Delta t_{\!\mathrm{eff}}\!\sim\!\Delta t/\tau\) up to order‑one factors set by the measured autocorrelation.
...
\paragraph{Scope (Stokes and frame closure).}
Work on a \frame, source‑free annulus around a compact source in \(n{=}3\).
...
(i) Far‑field boundary outside the source; (ii) \emph{low‑frequency window}: \(\omega r\ll c_I\) so wave export (\S\ref{waves}) is negligible;
