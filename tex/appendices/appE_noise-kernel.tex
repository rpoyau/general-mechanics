...
Readout mappings to measured sectors are stated once in §\ref{operationalization} and are not repeated here.
...
appropriate projection). Sector scales, if needed, are handled once in §\ref{operationalization}.
...
\subsection{Boundary/patch estimator: variance floor}
\label{app:noise-kernel:boundary-floor}

Specializing to inward Poisson arrivals across a patch \(A\) over a window \(\Delta t\), the
counting variance implies the local bound used in \ref{corollary:noise},
\begin{equation}
...
Via the one–datum mapping in §\ref{operationalization},
...
\eqref{app:noise-kernel:eq:EI-bound}, up to the (single) sector factor supplied in §\ref{operationalization}.
...
(§\ref{corollary:info-gas}, §\ref{corollary:frame-boundary}), the directed flux \(\Phi\)
...
A_3(r)=4\pi r^2 .
\label{app:noise-kernel:eq:stokes-static}
\end{equation}
...
\end{equation}
This is a labeling convention on the family (as in §\ref{operationalization}); it introduces no new constants.
...
(i) Far‑field boundary outside the source; (ii) quasistatic band: \(\omega\ll c_I/r\) so wave export (§\ref{waves}) is negligible;
...
(§\ref{corollary:info-gas}, §\ref{corollary:frame-boundary}). SI forms can be introduced by an external identification if needed.
...
Stokes/continuity statements used here: App.~\ref{app:forms-stokes}. Boundary geometry factors: App.~\ref{app:nD-screens}.
