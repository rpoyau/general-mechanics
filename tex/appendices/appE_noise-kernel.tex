...
mean continuity (from the Stokes identity) (App.~\ref{app:forms-stokes}, \eqref{appB:eq:stokes}–\eqref{appB:eq:local}). Let \(X\) denote
...
appropriate projection). Sector scales, if needed, are handled once in \S\ref{sec:operationalization}.
...
\end{equation}
identical to \eqref{eq:c6-var-EI}. Shot–noise specialization: \(F_A\to1\) and
\(\Delta t_{\!\mathrm{eff}}\to\Delta t\).
Via the single readout mapping in \S\ref{sec:operationalization},
...
This matches \eqref{eq:c6-var-EI} upon identifying \(\Delta t_{\!\mathrm{eff}}\!\sim\!\Delta t/\tau\) up to order‑one factors set by the measured autocorrelation.
...
\paragraph{Scope (Stokes + static closure).}
...
This is read from the boundary via Stokes.
...
(i) Far-field screen outside the source; (ii) quasistatic band: \(\omega\ll c_I/r\) so wave export (C4, \S\ref{corollary:info-em}) is negligible;
...
(Corollaries~\ref{corollary:info-gas}–\ref{corollary:static-screen}). SI forms can be introduced by an external identification if needed.
...
Poisson counting and renewal processes background appear in Kingman~\cite{Kingman1993}.
The Stokes/continuity identities used here are recalled in App.~\ref{app:forms-stokes}; geometric screen factors in App.~\ref{app:nD-screens}.
% ======================================================================
