% ======================================================================
% appA_kick.tex — Appendix A: Kick (Kz) units & Kram anchors
% ======================================================================
...
by dividing SI units by \(h\). \textbf{Kz is a bookkeeping convention (SI divided by \(h\)); results are unit‑agnostic.}
This keeps identities and bounds in rate form and avoids introducing additional constants in intermediate equations.
Pointers: boundary geometry (App.~\ref{app:nD-boundaries}); noise kernels and covariance (App.~\ref{app:noise-kernel});
the one‑datum sector mapping within this appendix (\S\ref{app:kick:calibrations}).
...
\textbf{Counts are in trits (default).} Conversion:
\[
S_{\rm nats}=(\ln 3)\,S_{\rm trits}.
\]
Only explicit \(\ln\) factors appear in time–information bounds. If boundary counts are recorded in trits,
\begin{equation}
...
The informational field is \(\mathbf E_I:=-\nabla\Phi_I\), with \emph{frame} linear closure
...
A_n(r)=S_{n-1}\,r^{\,n-1}\ \ \text{(App.~\ref{app:nD-boundaries})}.
...
\end{equation}
In the lossless \emph{frame} regime \(\Phi(r)=\Phi_\star\), so \(Y(r)=Y_\star\,(r_\star/r)^{\,n-1}\).
...
temperature \(T^{(\mathrm{Kz})}\), area \(A\), time \emph{windows} of duration \(\Delta t\), and reliability \(\varepsilon\)).
The time–information bounds used in the body (\S\ref{corollary:time-info}) combine three independent limits:
\begin{equation}
...
Apps.~\ref{app:forms-stokes}, \ref{app:nD-boundaries}, \ref{app:noise-kernel}.
