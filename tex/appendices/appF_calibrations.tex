% ======================================================================
% appF_calibrations.tex — Appendix F: One‑datum calibrations (frame readouts)
% ======================================================================

\section{One‑datum calibrations (frame readouts)}
...
% ------------------------- F.1 Recap -----------------------------------
\subsection{Recap: frame boundary law}
\label{app:calibrations:recap}

On a frame, in a source‑free annulus (\(\sigma=0\)), the \emph{directed boundary flux} is constant:
\[
\Phi(r)\;:=\;\int_{\Sigma(r)} \mathbf j\!\cdot n\,dA\quad\text{is independent of }r.
\]
...
\qquad
A_n(r)=S_{n-1}\,r^{\,n-1}\ \ \text{(App.~\ref{app:nD-boundaries}, eq.~\eqref{app:nD-boundaries:eq:areas}).}
...
\end{equation}
In the lossless frame case (\(\Phi(r)=\Phi_\star\)),
\begin{equation}
\boxed{\qquad
...
% ------------------------- F.7 Cones/apertures and confinement ---------
...
In guides/sheets (\S\ref{corollary:dimensional}), use the effective cross‑section or
...
Wave transport and envelopes are collected in \S\ref{waves}. Readouts inherit wave envelopes via
...
and (iii) the geometric areas \(A_n(r)\) from App.~\ref{app:nD-boundaries} (eq.~\eqref{app:nD-boundaries:eq:areas}).
