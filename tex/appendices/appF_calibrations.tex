...
\end{equation}
Naming $\mathcal Q$ as $M$ or $Q$ is \emph{bookkeeping}; no new constants are introduced.
...
In homogeneous, source‑free segments the exported power is constant and the far‑field envelope obeys
...
% ------------------------- F.9 Bit‑density bookkeeping -----------------
\subsection{Bit‑density parameterizations (bookkeeping only)}
\label{appF:bitdensity}

One may write $N(r)=\alpha\,A_n(r)$ (bits on a screen) and a per‑bit hit rate $f(r)$ with $\Phi(r)=f(r)\,N(r)$.
Conservation then fixes $f(r)\propto 1/A_n(r)$, so only the \emph{product} $f\alpha$ matters and can be absorbed into
the single calibration via \eqref{appF:eq:one-datum}. Hence $\alpha$ is a convenient parameterization, not an extra constant.
