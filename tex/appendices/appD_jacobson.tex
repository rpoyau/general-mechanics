% ======================================================================
% app_jacobson.tex — Appendix D: Local-horizon (Jacobson-style) calibrated sketch
% ======================================================================
...
relation used in the main text as a \emph{consistency tool}, not as an axiom.
...
channel (same one‑datum sector calibration as in statics). The energy (heat) flux through the horizon patch is
\begin{equation}
\delta Q
\;=\;
...
\qquad
\text{(same one‑datum sector calibration as in statics).}
...
Equation \eqref{app:jacobson:eq:calibrated} is the focusing consistency used in the body
(see \eqref{corollary:geometry:eq:info-einstein}); no separate coupling constant is introduced.
...
and App.~\ref{app:forms-stokes}, eqs.~\eqref{app:forms-stokes:eq:cap-side}–\eqref{app:forms-stokes:eq:cut}).
\item \textbf{Calibration unity.} The same one‑datum calibration that ties frame boundary amplitudes to the informational field
is used here to fix \((s_H^{(\mathrm{cal})},T_H^{(\mathrm{cal})})\), ensuring the equality \eqref{app:jacobson:eq:calibrated} carries no additional constants.
\item \textbf{Use.} Relation \eqref{app:jacobson:eq:calibrated} is employed as a \emph{consistency tool}
on the same manifold region and boundary; it is not taken as an axiom (see §\ref{corollary:geometry} for the operational use of focusing and area evolution).
\end{itemize}

\medskip
\noindent\emph{Summary.}
Local Clausius, together with Raychaudhuri focusing, yields a calibrated equality between
\(R_{ab}k^a k^b\) and \(\mathcal T^{(\mathrm{cal})}_{ab}k^a k^b\). This mirrors the frame boundary laws on the
same region and supports the geometry‑as‑tool perspective (see §\ref{corollary:geometry}).
...
These are provenance pointers; the equality here is calibrated and used as a consistency tool on the same region.
