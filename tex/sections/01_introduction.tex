...
Here \(j\) is identified with the \emph{entropy flux} on the boundary (\(j\equiv j_S\)); its normal component \(j_n\) has units
...
A heat mapping is optional: on a boundary, \(q_n = T\,j_n\) and over a window \(Q=\int T\,j_n\)
(introduced formally in Corollary~\ref{corollary:continuity:eq:clausius-local}).

\paragraph{Thermodynamics and gravity (same source family).}
Thermodynamics and gravity are read from the \emph{same} informational spine: the boundary entropy current \(j\)
(fixed by the frame inverse‑area rule) and its wave export on windows (fixed by conserved export). After a single
calibration datum, \emph{mass} and \emph{kinetic/radiation energy} label the same source family in predictions
(\S\ref{corollary:info-gas}–\S\ref{corollary:frame-boundary}, \S5.9). No extra constants are carried through the algebra; all
sector readouts follow from the same \(j\) on the same manifold.
...
This identity is applied in two complementary ways:
...
\[
\Phi=\int_{\Sigma} \mathbf j\!\cdot\!\mathbf n\,\mathrm dA
\]
...
\emph{Wave export/envelopes—see \S\ref{waves}.}
On windows that are linear, time‑invariant, and causal, measured dispersion \emph{may be compared}
to a Causality–Dispersion (KK‑window) relation \emph{as a local witness} (\S\ref{waves:causality-dispersion}).
