...
$j\equiv j_S$ (entropy current, nats/time), $i\equiv s$ (entropy density, nats/vol); logs are displayed in trits (\(\text{nats}=\ln 3\times\text{trits}\)); with boundary temperature \(T\), the heat flux is \(q:=T\,j_S\).
...
Stokes identity is $d(*j)=\sigma\,\mathrm{vol}$ (continuity).\footnote{The local and cut forms appear in
\eqref{eq:local-continuity} and \eqref{eq:cut-identity} (see D1).}
...
\paragraph{Moment slices (caps + side).}
The time‑extended boundary $\partial(U\times[t_0,t_1])$ decomposes into a top cap, a bottom cap, and a side
(worldtube). The cap–side formula is recorded in App.~\ref{app:forms-stokes}, Eq.~\eqref{appB:eq:cap-side};
...
On an open set $W\subset U$ with production density $\sigma|_W\equiv 0$ (source‑free), and with
the side of a narrow tube everywhere tangent to $\mathbf j$, the directed tube flux
...
(App.~\ref{app:forms-stokes}). \emph{A closure‑agnostic template is stated in C4, where Info--EM is one realization.}

\subsection{Units and calibration (once).}
Kz units are used (App.~\ref{app:kick}) so energies and powers are rates. The abstract informational field
is mapped to measured sectors (gravitational, electric, thermal) by a \emph{one‑datum} linear calibration
stated once (App.~\ref{app:calibrations}); no bare constants are carried through the algebra.
...
\begin{enumerate}\itemsep2pt
  ...
  \item Write the Stokes identity $d(*j)=\sigma\,\mathrm{vol}$ and, when needed, the \emph{cut identity} on $U$
        (D1; \eqref{eq:stokes-identity}–\eqref{eq:cut-identity}).
...
and canonical rates (D4: \eqref{eq:canonical}). The ladder then develops: relational info‑gas and static tube/screen
