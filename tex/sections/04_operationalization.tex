...
Wave statements are centralized in \S\ref{waves}. \emph{No wave equations appear here.}
...
\emph{W‑Lin[\(\varepsilon\)]}, \emph{W‑Stat[\(\varepsilon\)]}, \emph{W‑Ret[\(\varepsilon\)]}: local linearity, stationarity, and retardedness checks on the window (used when forming transfer functions; if satisfied, the Causality–Dispersion comparison may be invoked, \S\ref{waves:causality-dispersion}).\newline
...
% ----------------------------- 4.3 -------------------------------------
\subsection{Wave windows (pointer only; no equations here)}
...
capacity (\textbf{trits per time; trits are the default}). Then the resource–limited bounds are
\begin{equation}
...
\end{equation}
in \eqref{operationalization:limits:eq:rate}–\eqref{operationalization:limits:eq:time}. The \(P/T\) term is unchanged.
...
\begin{enumerate}\itemsep2pt
  ...
  \item Calibrate with the same \(c_{\rm sec}\) and quantify bounds via
        \eqref{operationalization:limits:eq:rate}–\eqref{operationalization:limits:eq:time} for the chosen reliability \(\varepsilon\).
...
for wave transport/export see \S\ref{waves} (no equations here) and App.~\ref{app:forms-stokes};
