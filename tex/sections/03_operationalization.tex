...
heat $\delta Q$ $\to$ informational bookkeeping $\Theta\,\delta S_I$ on the boundary (calibrated by one datum per sector).
...
For moving screens replace it by the instantaneous $A_{n}(r,t)$ and use the worldtube cap+side form with side integrand $\mathbf j\!\cdot\!\mathbf n - i\,v_n$ (App.~\ref{app:forms-stokes}).

% --- Operational witnesses (local, optional) ---
\paragraph{Operational witnesses (local, optional).}
\emph{W-FluxConst[$\varepsilon$]}: on a static annulus $W$ with $\sigma|_W\equiv 0$ (source‑free), the directed screen flux is invariant across nested screens (C1, \S\ref{corollary:static-screen}).
\emph{W-Narrow[$\varepsilon$]}: on a propagation segment, exported power is spectrally concentrated near a carrier (C4, \S\ref{corollary:info-em}).
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Resource budgets.} Area $A$, time window $\Delta t$, power budget $P$,
        ...
        for coding/decoding.
  \item \textbf{Noise record (optional).} If needed, a measured noise kernel $\mathcal N$ for
        fluctuations (App.~\ref{app:noise-kernel}) or simply the shot–noise witness used in C6 (\S\ref{corollary:noise}).
\end{enumerate}
...
This statement is \emph{windowed and data–driven}; no additional statistical postulates are assumed.
...
\begin{enumerate}\itemsep2pt
  ...
  \item Apply \eqref{eq:stokes-identity} and the cut identity \eqref{eq:cut-balance} with $\sigma=0$ on the annulus to identify
...
A 3D spherical screen of radius $r$ has area $A_3(r)=4\pi r^2$.
On a static annulus with $\sigma\approx 0$ (source‑free to measurement precision) and directed tube flux $\Phi$,
\begin{equation}
\langle j_n(r)\rangle=\frac{\Phi}{A_3(r)}=\frac{\Phi}{4\pi r^2} .
\end{equation}
With the snapshot closure \eqref{eq:static-closure}, the informational field amplitude reads
\begin{equation}
\big|\mathbf E_I(r)\big|=\frac{1}{\kappa}\,\frac{\Phi}{A_3(r)}=\frac{1}{\kappa}\,\frac{\Phi}{4\pi r^2}.
\end{equation}
...
for wave transport and power export see App.~\ref{app:forms-stokes} and standard EM texts (e.g.\ Jackson~\cite{Jackson1999});
