...
\section{Tier-1 Corollary I: Information-Field Divergence}
\label{sec:tier1_maxwell}\label{sec:info_field_div}\label{sec:tier1}
 Grounding in Stokes (3+1): see Sec.~\ref{subsec:gst_3p1}.
...
\paragraph{Source equation (Gauss–Ampère analogue).}
Taking the divergence of $F_{\mu\nu}$ gives
\[
\partial_\mu F^{\mu\nu}
= \Box\,\Lambda^\nu - \partial^\nu(\partial\!\cdot\!\Lambda)
\;\coloneqq\; J^\nu_{\mathrm{info}} \, .
\]
We \emph{define} the informational source four–current by
$J^\nu_{\mathrm{info}} := \partial_\mu F^{\mu\nu}$.
Its relation to $(\rho,\mathbf J)$ is fixed by the continuity identity
$\partial_t \rho + \partial_i J^i = \sigma$ and boundary data; there is no identity that enforces
$J^\nu_{\mathrm{info}} = \pm\,\partial^\nu \rho$.
