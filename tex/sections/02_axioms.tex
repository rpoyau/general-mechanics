...
cited; see \S\ref{corollary:info-gas} and Corollary~\ref{corollary:continuity}.)
...
\paragraph{Boundary identification (information \(\equiv\) entropy; thermal mapping).}
We use \emph{entropic flux} as the primitive: \(j\equiv j_S\) (entropy current), with normal component
\(j_n=\mathbf j\!\cdot\!\mathbf n\) (units: count/(area·time); log‑base fixes count unit—nats or trits). The corresponding
density is \(i\equiv s\) (nats/vol). Where a boundary temperature field \(T\) is assigned, the heat flux is
\(\mathbf q := T\,\mathbf j_S\) (Kz). Operational window integrals (counts and window‑averaged currents) are defined
once in \S\ref{operationalization}. The axioms themselves are temperature‑free.
...
If the boundary moves, the side contribution uses the integrand \(\mathbf j\!\cdot\!\mathbf n - i\,v_n\), and the same
...
channel. This closure is used on frames to derive boundary/sector relations; time‑extended segments and \emph{wave} transport are
treated separately (see \S\ref{waves}); no wave equations are part of the axioms.
...
(see \S\ref{operationalization}). Define the conjugate one‑form \(\lambda:=\lambda_a\,dR^{(a)}\) with components
...
(2) Propagation closures and wave transport are introduced and used in \S\ref{waves} (with dimension/confinement
summarized in \S\ref{corollary:dimensional}); they are \emph{not} additional axioms.\\
