...
for waves, the exported‑power/envelope statements collected once in \S\ref{waves}.
...
\(\Phi\) is constant (\S\ref{corollary:info-gas}), so the \textbf{frame} mean and amplitude follow
(\S\ref{corollary:frame-boundary})
\begin{equation}
...
For \textbf{wave} segments in homogeneous media, \emph{exported power is radius‑independent on source‑free windows
and the far‑field envelope scales as} \(\langle|\mathbf E_I(r)|\rangle \propto [A_n(r)]^{-1/2}\) (see \S\ref{waves}).
For spherical/conical boundaries this yields \(\langle|\mathbf E_I|\rangle\propto r^{-(n-1)/2}\).
...
Here \(A_n(r)\) is the \(n\)D boundary area (App.~\ref{app:nD-boundaries}). Frame scaling follows \eqref{corollary:dimensional:eq:frame-scale};
wave envelope exponents are summarized from \S\ref{waves}.}
...
\begin{itemize}\itemsep2pt
  ...
  \textbf{Waves:} \(\langle|\mathbf E_I|\rangle\sim r^0\) along the guide; mode power is exported as in \S\ref{waves}.
...
\eqref{corollary:dimensional:eq:frame-scale}. This is the sector (tube) form
of the same conservation rule (\S\ref{corollary:info-gas}).
...
These follow directly from the Stokes identity, the frame linear closure, and the wave export/envelope statements in \S\ref{waves}, with no additional postulates.

\medskip
\noindent\emph{Literature note.}
Background geometry/areas and cone/tube forms are summarized in App.~\ref{app:nD-boundaries}; the exported‑power
and envelope statements are recalled in \S\ref{waves} and App.~\ref{app:forms-stokes}. For standard expositions, see
