...
(for waves) the local balance (Poynting) identity \eqref{waves:eq:local-balance} (see \S\ref{waves}; App.~\ref{app:forms-stokes}).
...
radius‑independent exported power (see \S\ref{waves}),
\begin{equation}
P(r):=\int_{\Sigma(r)}\!\big\langle \mathbf S_I\big\rangle\!\cdot n\,dA=\text{const in }r ,
\label{corollary:dimensional:eq:power-const}
\end{equation}
...
\quad\text{(cf.\ \S\ref{waves}, eq.~\eqref{waves:eq:envelope})}.
\label{corollary:dimensional:eq:wave-envelope}
\end{equation}
...
wave envelopes follow \eqref{corollary:dimensional:eq:power-const}–\eqref{corollary:dimensional:eq:wave-envelope}.}
...
\begin{itemize}\itemsep2pt
  ...
  \textbf{Waves:} \(\langle|\mathbf E_I|\rangle\sim r^0\) along the guide; mode power is exported with \eqref{corollary:dimensional:eq:power-const}.
...
\eqref{corollary:dimensional:eq:frame-scale}–\eqref{corollary:dimensional:eq:wave-envelope}. This is the sector (tube) form
...
statement is recalled in \S\ref{waves} and App.~\ref{app:forms-stokes}. For standard expositions, see
