...
We analyze open informational channels driven across a bounded region \(V\) along a narrow tube
\(\mathcal T\) with inlet cross–section \(\Sigma_{\rm in}\) and outlet \(\Sigma_{\rm out}\).
Let
\[
...
\]
denote the inward and outward directed fluxes of the channel. The cut balance
\eqref{eq:cut-balance} on the tube volume reads
\begin{equation}
...
\paragraph{(i) Transparent transit (no production).}
Assume \(\sigma=0\) in \(\mathcal T\) and a directed speed profile \(v(s)>0\) along the tube coordinate
\(s\in[0,L]\). The transit time is
\begin{equation}
...
Suppose interactions remove directed flux at a local rate \(\mu(s)\ge 0\) (per unit length).
Let \(\Phi(s,t):=\int_{\Sigma(s)}(\mathbf j\!\cdot n)_+\,dA\) be the directed tube flux and assume advection
with speed \(v(s)\). A first–order transport model gives
\begin{equation}
...
\(I_{\mathcal T}\) and/or as nonnegative production \(\Pi_{\mathcal T}\ge 0\) (cf.\ Axiom~\ref{axiom:reos} for cyclic–cost monotonicity).
...
\(\mathcal T\times[t_0,t_1]\) yields the same cut form \eqref{eq:c3-cut}.
...
(see \S\ref{subsec:op-calibration}; App.~\ref{app:calibrations}), the corresponding power/time units are in Kz (App.~\ref{app:kick}).
The static closure \(\mathbf j=-\kappa\,\mathbf E_I\) then maps \(\langle j_n\rangle\) to field amplitudes along the tube.

\paragraph{Remarks.}
(i) The transparent–channel limit is a practical way to identify transit time by cross–correlating
...
(iii) Apertures and cones are handled by replacing \(A(s)\) with the sector area \(A_\Delta(r)\) (App.~\ref{app:nD-screens}).
...
Cross–sectional amplitudes always follow the \(1/A\) rule, modulated by the exponential survival
along the tube. Series segments add delays and multiply survivals, independent of the choice of frame.

\medskip
\noindent\emph{Literature note.}
The advection–attenuation model \eqref{eq:c3-transport} is the standard first–order transport (Beer–Lambert–type) equation; see, e.g., Evans \cite{Evans2010}. Stokes/cut forms used here are recalled in App.~\ref{app:forms-stokes}; sector geometry (areas) in App.~\ref{app:nD-screens}; units and rate conventions in App.~\ref{app:kick}.
