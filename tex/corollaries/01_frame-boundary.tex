...
Work on a single frame (time-frozen) with $\partial_t i=0$. On an open annulus $W\subset U$ with $\sigma|_{W}\equiv 0$
...
(see \eqref{axioms:stokes:identity:eq:cut})—$\Phi(r)$ is independent of $r$ across nested screens in $W$.

\paragraph{Single-frame linear closure.}
Using the static closure \eqref{axioms:stokes:frame:eq:closure},
\[
\mathbf E_I:=-\,\nabla\Phi_I,
\qquad
...
For a spherically symmetric screen in $n$ spatial dimensions (App.~\ref{app:nD-boundaries}),
\[
...
\end{equation}
Thus single-frame informational-field amplitudes scale as $r^{-(n-1)}$.
...
Screen areas and Green–function normalizations appear in App.~\ref{app:nD-boundaries};
