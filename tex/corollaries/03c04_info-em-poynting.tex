...
On channels that support wave-like transport, specialize to the informational electrodynamics
\begin{equation}
dF_I=0,\qquad dH_I={*}j,\qquad H_I=\chi:F_I,\qquad F_I=dA_I ,
\label{eq:c4-ied}
\end{equation}
where $A_I$ is a $1$-form potential, $F_I$ the $2$-form field, and $\chi$ a linear, local
constitutive map on the segment considered. This is a \emph{closure choice} (not an axiom of the
framework). When a causal, time-invariant model for $\chi$ is adopted—or when retarded response
is observed on a window—the measured/adopted dispersion may be \emph{compared} against the
Kramers--Kronig Hilbert–transform relations recorded in App.~\ref{app:forms-stokes},
eq.~\eqref{appB:eq:KK}.
