...
Assume local fluctuations around the canonical drift admit a Gaussian kernel with positive‑definite
diffusivity matrix $D(q,t)$ and a scalar rate field $U(q,t)$.
...
\label{eq:c12-weight}
\end{equation}
The density $\rho(q,t)$ obeys a Fokker--Planck form
\begin{equation}
\partial_t \rho
=
-\nabla\!\cdot(v\,\rho)
...
\label{eq:c12-fp}
\end{equation}
with $D$ tied to the counting noise kernel of App.~\ref{app:noise-kernel} (see C6).
...
$\dot q=M^{-1}P$ and $\dot P=-\nabla U$ (cf.\ C9 for the Newtonian specialization).
...
If an informational 1‑form $A_I$ is present (C4), minimal coupling enters by
\begin{equation}
P\ \mapsto\ P-A_I(q,t)
\quad\text{in}\quad
