...
\end{equation}
For wave amplitudes in homogeneous segments (see \S\ref{waves}, eq.~\eqref{waves:eq:envelope}),
...
(\S\ref{operationalization:calibration}) ties the focusing term to a calibrated stress component:
\begin{equation}
R_{ab}\,k^a k^b
\;\calibeq\;
...
the same datum that fixes the frame boundary law (\S\ref{corollary:frame-boundary}) fixes the focusing response
...
(\S\ref{corollary:frame-boundary}) and the wave envelope in \S\ref{waves}.
...
With one calibration shared with the frame boundary law, the focusing term can be related to a calibrated stress
...
in App.~\ref{app:nD-boundaries}. Frame and wave amplitude scalings referenced here are established in
\S\ref{corollary:frame-boundary} and \S\ref{waves}.
