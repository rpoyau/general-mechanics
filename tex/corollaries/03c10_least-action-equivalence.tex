% ======================================================================
% 03c10_least-action-equivalence.tex — C10: Least‑action equivalence (derived)
% ======================================================================

\subsection{C10: Least‑action equivalence (derived, not assumed)}
\label{corollary:least-action}
This corollary show that the usual least‑action (Euler–Lagrange) equations are \emph{equivalent} to the
...
\label{eq:c10-pc-form}
\end{equation}
and impose the closure $d\Theta_{\mathcal R}=0$ on admissible rectangles in $(q,t)$.
In local coordinates this is equivalent to the canonical rate equations
\begin{equation}
\dot q^i=\partial_{P_i}H,
\qquad
\dot P_i=-\,\partial_{q^i}H ,
\label{eq:c10-canonical}
\end{equation}
cf.\ the axiom‑level statement \eqref{eq:pc-form}–\eqref{eq:canonical}.
...
\label{eq:c10-action}
\end{equation}
so \eqref{eq:c10-stokes} implies
\begin{equation}
\delta\mathcal S[q]\;=\;0 \quad \text{for fixed endpoints } q(t_0),\,q(t_1).
\label{eq:c10-deltaS0}
\end{equation}

\paragraph{Euler–Lagrange $\Leftrightarrow$ canonical rates.}
Stationarity of \eqref{eq:c10-action} yields the Euler–Lagrange equations
\begin{equation}
...
the canonical system \eqref{eq:c10-canonical}. Thus \eqref{eq:c10-canonical} are
Hamilton’s equations for the generator $H$:
\begin{equation}
\dot q^i=\partial_{P_i}H, \qquad \dot P_i=-\,\partial_{q^i}H ,
\label{eq:c10-Hamilton}
\end{equation}
...
equations; a $3{+}1$ split reduces to \eqref{eq:c10-canonical}. See App.~\ref{app:ddw}.
