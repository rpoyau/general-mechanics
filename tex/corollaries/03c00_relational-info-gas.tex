...
All corollaries below follow directly from the generalized Stokes identity \(d(*j)=\sigma\,\mathrm{vol}\) and the
snapshot closure \(\mathbf j=-\kappa\,\mathbf E_I\), plus the optional Info--EM specialization where stated.
Standard identities (e.g., Poynting) are recalled in Apps.~
ef{app:forms-stokes}--
ef{app:jacobson} and are used only when explicitly invoked in the relevant corollary.
...
\label{eq:c0-cut}
\end{equation}
which is \eqref{eq:cut-identity} in the axioms. The static, linear snapshot closure is
\begin{equation}
\mathbf E_I:=-\nabla\Phi_I,
\qquad
