% ======================================================================
% corollaries/09_thermo-newton.tex — Thermodynamics → Newtonian rate laws
% ======================================================================
...
\end{equation}
evaluating Stokes on a rectangular \(2\)-surface \(\Sigma\) spanned by two nearby paths between \(t\) and \(t{+}dt\)
\[
\oint_{\partial\Sigma}\Theta_{\mathcal R}\;=\;\int_{\Sigma} d\Theta_{\mathcal R}\;=\;0,
\]
gives the canonical rate equations
\begin{equation}
\dot q = \partial_{P}H,
\qquad
\dot P = -\,\partial_{q}H .
...
\end{equation}
These match \eqref{axioms:path-rate:eq:pc-form}–\eqref{axioms:path-rate:eq:canonical}.
...
In the REOS language \S\ref{axioms:reos}, boundary/control variations \(R^{(a)}\) with price \(\lambda_a\) read
\(d\rho=-\lambda_a\,dR^{(a)}\). When a control field induces a spatial price gradient, the force on a probe
is the \(q\)-gradient of a potential, i.e.\ \(\mathbf f=-\partial_q U\) in
