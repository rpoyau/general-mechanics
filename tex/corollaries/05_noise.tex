...
normal \emph{current density} as
\begin{equation}
...
Let \(N_A(\Delta t)\) be the inward \emph{counts} on \(A\) over the window \(\Delta t\).
Define the windowed Fano factor
\begin{equation}
F_A \;:=\; \frac{\mathrm{Var}(N_A)}{\mathbb E[N_A]},
\label{corollary:noise:eq:mean-fano}
\end{equation}
and an \emph{effective window} \(\Delta t_{\!\mathrm{eff}}\le \Delta t\) obtained from the
count autocorrelation on the same record (App.~\ref{app:noise-kernel}). Any boundary estimator of the
patch‑mean current density admits the operational benchmark
\begin{equation}
\mathrm{Std}\!\big(\widehat{j}_A\big)
\;\approx\;
...
\begin{equation}
\boxed{\quad
\mathrm{Var}(\widehat{E}_I)\ \gtrsim\ \frac{F_A\,j_A}{\kappa^{2}\,|A|\,\Delta t_{\!\mathrm{eff}}},
\qquad
...
\begin{equation}
\sigma_{E_I}(\Delta t)\ \gtrsim\ \frac{1}{\kappa}\sqrt{\frac{j_A}{|A|\,\Delta t}} .
...
\begin{equation}
\mathrm{Var}(\widehat{j}_A)\ \gtrsim\ \frac{j_A}{|A|}\,\frac{\tau}{\Delta t},
\qquad
\sigma_{E_I}\ \gtrsim\ \frac{1}{\kappa}\sqrt{\frac{j_A}{|A|}\,\frac{\tau}{\Delta t}} .
...
\begin{equation}
\sigma_{g}\ \gtrsim\ \frac{c_M}{\kappa}\sqrt{\frac{F_A\,j_A}{|A|\,\Delta t_{\!\mathrm{eff}}}},
\qquad
\sigma_{E}\ \gtrsim\ \frac{c_Q}{\kappa}\sqrt{\frac{F_A\,j_A}{|A|\,\Delta t_{\!\mathrm{eff}}}} .
...
\(\sigma_{E_I}\propto \sqrt{j_A/(|A|\Delta t_{\!\mathrm{eff}})}\).
