...
\paragraph{Setup (time frozen, source free).}
Work on a snapshot with $\partial_t i=0$ and no production in the region, $\sigma=0$.
Let $j=J^\mu dx_\mu$ be the informational/entropic current and define the directed flux through a closed
screen $\Sigma(r)$ of radius $r$ by
\[
\Phi(r):=\int_{\Sigma(r)} \mathbf j\!\cdot n\,dA.
\]
By the Stokes identity \eqref{eq:stokes-identity} with $\sigma=0$, $\Phi(r)$ is independent of $r$.
...
\end{equation}
Thus static informational fields decay as $r^{-(n-1)}$.
...
\paragraph{Units and bits/nats.}
Counts on $\partial U$ may be recorded in bits; the entropy flux in nats is $j_S=(\ln2)\,j_{\rm bits}$.
The law \eqref{eq:c1-screen-law} is unit–agnostic; only the numerical value of $\kappa$ changes with the
bit/nat convention (global unit policy is stated once elsewhere).
