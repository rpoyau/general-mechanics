...
\end{equation}
with \(D\) tied to the counting noise kernel of App.~\ref{app:noise-kernel} (see §\ref{corollary:noise}).
...
\(\dot q=M^{-1}P\) and \(\dot P=-\nabla U\) (cf.\ §\ref{corollary:thermo-newton}).
...
If an informational \(1\)-form \(A_I\) is present (§\ref{waves}), minimal coupling enters by
\begin{equation}
P\ \mapsto\ P-A_I(q,t)
\quad\text{in}\quad
...
Heuristically, larger counting noise (larger \(D\) in §\ref{corollary:noise}/App.~\ref{app:noise-kernel})
