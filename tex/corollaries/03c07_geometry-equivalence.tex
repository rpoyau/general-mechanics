...
\end{equation}
In a steady, source–free tube the directed flux $\Phi$ is constant and the mean normal current obeys
$\langle j_n\rangle=\Phi/A$. Therefore
\begin{equation}
\frac{d}{d\lambda}\ln \langle j_n\rangle \;=\; -\,\theta .
\label{eq:c7-spread-static}
\end{equation}
For wave amplitudes in homogeneous segments (see C4, \S\ref{corollary:info-em}), $\langle |\mathbf E_I|\rangle \propto A^{-1/2}$, hence
\begin{equation}
...
On the \emph{same} region used for flux measurements, we employ a single sector calibration (see \S\ref{subsec:op-calibration}) that ties the focusing term
